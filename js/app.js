 ...
 */
    ...
    (() => {
        ...
        function splitShallow(splitToken, inputString) {
            if (!("string" === typeof splitToken || splitToken instanceof String)) throw new TypeError(`Expected splitToken to be string. Got ${typeof splitToken}.`);
            if (!("string" === typeof inputString || inputString instanceof String)) throw new TypeError(`Expected inputString to be string. Got ${typeof inputString}.`);
            const openingParenthesis = [ "(", "[", "{", "<" ];
            const closingParenthesis = [ ")", "]", "}", ">" ];
            let currentLevel = 0;
            const result = [];
            let buffer = "";
            for (let i = 0; i < inputString.length; i++) if (openingParenthesis.includes(inputString[i])) {
                buffer += inputString[i];
                currentLevel++;
            } else if (closingParenthesis.includes(inputString[i])) {
                buffer += inputString[i];
                currentLevel--;
            } else if (inputString[i] === splitToken && 0 === currentLevel) {
                result.push(buffer);
                buffer = "";
            } else buffer += inputString[i];
            result.push(buffer);
            return result;
        }
        function parseGradientString(gradientString) {
            if (!("string" === typeof gradientString || gradientString instanceof String)) throw new TypeError(`Expected gradientString to be a string. Got: ${typeof gradientString}`);
        ...
        function getPositionOnEllipse(width, height, angleDegrees) {
            if ("number" !== typeof width) throw new TypeError(`Expected width to be a number. Got ${typeof width}.`);
            if ("number" !== typeof height) throw new TypeError(`Expected height to be a number. Got ${typeof height}.`);
            if ("number" !== typeof angleDegrees) throw new TypeError(`Expected angleDegrees to be a number. Got ${typeof angleDegrees}.`);
        ...
        function lerp(value1, value2, time) {
            if ("number" !== typeof value1) throw new TypeError(`Expected value1 to be number. Got ${typeof value1}`);
            if ("number" !== typeof value2) throw new TypeError(`Expected value2 to be number. Got ${typeof value2}`);
            if ("number" !== typeof time) throw new TypeError(`Expected time to be number. Got ${typeof time}`);
            if (time < 0) return value1;
            if (time > 1) return value2;
            return (1 - time) * value1 + time * value2;
        }
        function aqerp(minAngle, maxAngle, maxSpeed) {
            if (maxAngle < minAngle) throw new Error(`Expected maxAngle > minAngle. Got: ${maxAngle} (maxAngle) < ${minAngle} (minAngle).`);
            ...
            return () => {
                if (angle < minAngle) throw new Error(`Angle outside a range. Got ${angle} (angle) < ${minAngle} (minAngle)`);
                if (angle > maxAngle) return {
                    angle,
                    done: true
                };
                const normAngle = getNormalizedAngle(angle);
                speed = normAngle < halfRange ? lerp(.1, maxSpeed, normAngle / halfRange) : lerp(maxSpeed, .1, normAngle / halfRange - 1);
                angle += speed;
                return {
                    angle,
                    done: false
        ...
        function getCurrentBreakPointId() {
            const mediaQueries = breakPoints.map((bp => {
                const [min, max] = bp;
                let query = "";
                if (max === inf && 0 === min) throw new Error(`Expected either min or max to defined. Got min: ${min}, max: ${max}`);
                if (max === inf) query = `(min-width: ${min}px)`; else if (0 === min) query = `(max-width: ${max}px)`; else query = `(min-width: ${min}px) and (max-width: ${max}px)`;
                return query;
            }));
            for (let i = 0; i < mediaQueries.length; i += 1) {
                const mediaQuery = mediaQueries[i];
                if (window.matchMedia(mediaQuery).matches) return i;
            }
            throw new Error(`No breakpoint match. Client-width: ${window.innerWidth}, breakpoints: ${breakPoints}.`);
        }
        function addBreakPointChangeHandler(handler) {
            if (!(handler instanceof Function)) throw new TypeError(`Expected handler to be a Function. Got ${typeof handler}`);
        ...
        const injectToken = "{animated}";
        const maxSpeed = 5;
        const inf = Number.POSITIVE_INFINITY;
        const breakPoints = [ [ 0, 480 ], [ 480, 834 ], [ 834, 1300 ], [ 1300, inf ] ];
        ...
        class AnimateMoon {
            constructor(onAnimationStep, {title, subtitle, animateNumber}) {
                ...
                if (seconds < 1) throw new Error(`Expected seconds to be 1 or more. Got: ${seconds}`);
                this.animationFrame = 75;
                const unit = 1e3 * seconds / this.animationFrame;
                const increment = (end - start) / unit;
                if (start < 0) throw new Error(`Expected start >= 0. Got: ${start}`);
                if (end <= start) throw new Error(`Expected end > start. Got end (${end}) <= start (${start}) `);
                if (increment >= end - start) throw new Error(`Expected increment < (end - start). Got ${increment} (increment) >= (${end} - ${start}) (end - start)`);
                this.a = start;
                this.b = end;
                this.increment = increment;
                this.current = this.a;
                if (!(onAnimationStep instanceof Function)) throw new TypeError(`Expected onAnimationStep to be a function. Got: ${typeof onAnimationStep}`);
                this.onAnimationStep = onAnimationStep;
                if ("number" === typeof animateNumber.delay && animateNumber.delay) {
            ...
            injectValue(str) {
                if (!("string" === typeof str || str instanceof String)) throw new TypeError(`Expected str to be a string. Got: ${typeof str}`);
                return str.replace(injectToken, `${this.current.toFixed()}`);
            }
            makeInterval() {
                this.interval = setInterval((() => {
                    if (this.increment < this.b - this.current) this.current += this.increment; else {
                        this.current = this.b;
                        this.stop();
                    }
                    this.onAnimationStep({
            ...
            start() {
                if (!this.started) {
                    this.started = true;
                    this.current = this.a;
                    if (this.delay > 0) this.timeout = setTimeout((() => this.makeInterval()), 1e3 * this.delay); else this.makeInterval();
                }
            }
            stop() {
                if (this.started) {
        ...
        function createOrbit({width, height, gradientString, borderWidth, data: {angleDegrees, title, subtitle, animateNumber}, renderToElement}) {
            const ns = "http://www.w3.org/2000/svg";
            if ("number" !== typeof width) throw new TypeError(`Expected width to be number. Got: ${typeof width}`);
            if ("number" !== typeof height) throw new TypeError(`Expected height to be number. Got: ${typeof height}`);
            if ("number" !== typeof borderWidth) throw new TypeError(`Expected borderWidth to be number. Got: ${typeof borderWidth}`);
            if (!("string" === typeof title || title instanceof String)) throw new TypeError(`Expected title to be a string. Got ${typeof title}`);
            if (!("string" === typeof subtitle || subtitle instanceof String)) throw new TypeError(`Expected subtitle to be a string. Got ${typeof subtitle}`);
            if (!(renderToElement instanceof HTMLElement)) throw new TypeError(`Expected renderToElement to be HTMLElement. Got ${renderToElement.constructor.name}`);
            ...
            let counter = 1;
            if (null !== document.getElementById(gradientId)) {
                while (null !== document.getElementById(`${gradientId}-${counter}`)) counter++;
            ...
            counter = 1;
            if (null !== document.getElementById(maskId)) {
                while (null !== document.getElementById(`${maskId}-${counter}`)) counter++;
            ...
            Object.assign(moon.style, {
                width: `${2 * moonRadius}px`,
                height: `${2 * moonRadius}px`
            });
            ...
            if ("object" === typeof animateNumber && "number" === typeof animateNumber.start && "number" === typeof animateNumber.end && "number" === typeof animateNumber.seconds) {
        ...
        function Orbit() {
            ...
            if (!(container instanceof HTMLElement)) throw new TypeError(`Expected container to be HTMLElement. Got: ${typeof container}`);
            if (!(contentEl instanceof HTMLElement)) throw new TypeError(`Expected contentEl to be HTMLElement. Got: ${typeof contentEl}`);
            const calcOffset = () => {
                ...
                if (0 === getCurrentBreakPointId()) left = getValue(width) + getValue(paddingLeft);
                const offset = {
            ...
            orbits.forEach((({orbitData}) => {
                ...
                if (maxWidth <= 0) throw new TypeError("Expected maxWidth to be positive");
                if (maxHeight <= 0) throw new TypeError("Expected maxHeight to be positive");
            ...
            const interval = setInterval((() => {
                ...
                if (finished.every((({done}) => true === done)) && finished.length === orbits.length) clearInterval(interval);
            }), 100);
            return {
                detach: () => {
        ...
        window.addEventListener("load", (() => {
            ...
            if ("object" === typeof orbitHandle) addBreakPointChangeHandler((id => {
                orbitHandle.detach();
                orbitHandle = Orbit();
            }));
        }));
        ...
        function removeFieldTag(wrapperEl, ignoreNameAtrr = false) {
            ...
            const {fieldTag} = exchanger;
            const fieldTagStr = `${fieldTag}-`;
            if (inputEl.hasAttribute("name")) if (inputEl.name.startsWith(fieldTagStr)) inputEl.name = inputEl.name.replace(fieldTagStr, "");
        }
        function addFieldTag(wrapperEl, ignoreNameAttr = false) {
            const inputEl = validateWrapperEl(wrapperEl, ignoreNameAttr);
            const {fieldTag} = exchanger;
            const fieldTagStr = `${fieldTag}-`;
            if (inputEl.hasAttribute("name")) if (!inputEl.name.startsWith(fieldTagStr)) inputEl.name = fieldTagStr + inputEl.name;
        }
        function validateWrapperEl(wrapperEl, ignoreNameAttr = false) {
            if (!(wrapperEl instanceof Element)) throw new ElementNotFoundError("[param of switchFieldTag()] wrapperEl");
            const inputEl = wrapperEl.querySelector("input");
            if (!(inputEl instanceof Element)) throw new ElementNotFoundError("input");
            if (!inputEl.hasAttribute("name") && !ignoreNameAttr) throw new Error('input element is missing "name" attribute');
            return inputEl;
        }
        function enableElement(wrapperEl) {
            addFieldTag(wrapperEl, true);
            wrapperEl.classList.remove("hidden");
        }
        function disableElement(wrapperEl) {
        ...
        function filterObjKeys(obj, predicate) {
            if ("object" !== typeof obj) throw new TypeError("Expected obj to be object");
            const result = {};
            Object.keys(obj).filter(predicate).forEach((k => {
                result[k] = obj[k];
            }));
            return result;
        }
        function splitObj(obj, keyPredicate) {
            const negPred = k => !keyPredicate(k);
            return [ filterObjKeys(obj, keyPredicate), filterObjKeys(obj, negPred) ];
        }
        function applyObjValues(obj, fn) {
            Object.values(obj).forEach(fn);
        }
        function applyObjValuesByPred(obj, keyPredicate, applyFns) {
            splitObj(obj, keyPredicate).forEach(((objSplit, splitId) => {
                applyObjValues(objSplit, applyFns[splitId]);
            }));
        }
        class AddressCardView {
            constructor(sendReceiveModel, formElement) {
                if (!(sendReceiveModel instanceof send_receive)) throw new TypeError("Expected sendReceiveModel to be SendReceiveModel");
                this.model = sendReceiveModel;
                if (!(formElement instanceof Element)) throw new ElementNotFoundError(exFormId);
                this.wrapperElements = [ "card", "address", "copy-address", "copy-ua-card", "copy-usd-card" ].reduce(((accObj, className) => {
                    const wrapperEl = formElement.querySelector(`.${className}`);
                    if (!(wrapperEl instanceof Element)) throw new ElementNotFoundError(`.${className}`);
        ...
        const address_card = AddressCardView;
        async function exchangerPageLoad() {
            ...
            new address_card(youSendReceiveModel, formElement);
        }
